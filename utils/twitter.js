...
class TwitterBot {
  ...
  deleteMessage = (message) => {
    return new Promise((resolve, reject) => {
      this.client.delete('direct_messages/events/destroy', { id: message.id }, (error, data) => {
        if (!error) {
  ...
  uploadMedia = (filePath, type) => {
    return new Promise((resolve, reject) => {
      ...
      if (type === 'photo') {
        this.client.post('media/upload', { media_data: b64content }, (error, data) => {
          if (!error) {
            resolve(data);
        ...
        this.client.postMediaChunked({ file_path: filePath }, (error, data) => {
          if (!error) {
            resolve(data);
            console.log('[~] Media has been successfuly uploaded...');
          } else {
  ...
  tweetMessage = (message) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        this.client.post('statuses/update', payload, (error, data) => {
          if (!error) {
  ...
  getDirectMessage = (userId) => {
    return new Promise((resolve, reject) => {
      this.client.get('direct_messages/events/list', async (error, data) => {
        try {
          if (!error) {
            let lastMessage = {};
